...
resource "aws_codebuild_project" "codebuild" {
  ...
  build_timeout = "60"         #완료로 표시되지 않은 관련 빌드 시간 초과될 때까지 대기시간

  # Source: Github 경로 설정
  source {
    type            = "GITHUB"                               #빌드 소스 코드 저장소 유형
    location        = "https://github.com/Joonzales/app.git" #빌드 소스 코드 경로
  ...
  # 서비스 역할
  service_role = aws_iam_role.codebuild-role.arn #CodeBuild IAM 역할 리소스 이름
