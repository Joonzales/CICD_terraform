resource "aws_codedeploy_deployment_group" "codeployGroup" {  #CodeDeploy 배포 그룹
  app_name               = aws_codedeploy_app.codedeploy.name #애플리케이션 이름
  deployment_group_name  = "Project-DeployGroup"              #배포그룹 이름
  deployment_config_name = "CodeDeployDefault.ECSAllAtOnce"   #배포 구성 이름
  service_role_arn       = aws_iam_role.codedeploy-role.arn   #ecs Service

  # Codedeploy 배포 그룹과 연결
  ecs_service {
    cluster_name = aws_ecs_cluster.ecs-cluster.name #ECS Cluster Name
    service_name = aws_ecs_service.ecs-service.name #ECS Service Name 
  }
  ...
  load_balancer_info {       #로드밸런서 정보 
    target_group_pair_info { # 배포에 사용할 (애플리케이션/네트워크 로드 밸런서) 대상 그룹
      prod_traffic_route {   #프로덕션 트래픽 경로에 대한 구성
        listener_arns = [aws_lb_listener.albli-blue.arn]
      }
      test_traffic_route { #테스트 트래픽 경로에 대한 구성
        listener_arns = [aws_lb_listener.albli-green.arn]
      }
      target_group { #대상 그룹 쌍 내의 대상 그룹에 대한 구성 
        name = aws_lb_target_group.alb-blue.name
      }
      target_group { #대상 그룹 쌍 내의 대상 그룹에 대한 구성 
        name = aws_lb_target_group.alb-green.name
      }
    }
  }
}
